...
export default function Search() {
  ...
  const fetchAssets = async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        ...(filters.priceFilter !== 'all' && { priceFilter: filters.priceFilter }),
  ...
  const handleFilterChange = (key: keyof SearchFilters, value: string) => {
    ...
    if (newFilters.category !== 'All Categories') newSearchParams.set('category', newFilters.category);
    if (newFilters.priceFilter !== 'all') newSearchParams.set('price', newFilters.priceFilter);
    if (newFilters.sortBy !== 'popular') newSearchParams.set('sort', newFilters.sortBy);
  ...
  const AssetCard = ({ asset }: { asset: Asset }) => (
    <Card className={`group hover:shadow-lg transition-all duration-200 ${viewMode === 'list' ? 'flex' : ''}`}>
      <div className={`relative ${viewMode === 'list' ? 'w-48 flex-shrink-0' : 'aspect-video'} bg-gray-100 rounded-t-lg overflow-hidden`}>
        <img
          src={asset.image}
          alt={asset.title}
        ...
        {asset.isBestSelling && (
          <Badge className="absolute top-2 left-2 bg-green-500">Best Selling</Badge>
        )}
        {asset.isFeatured && (
      ...
      <CardContent className={`p-4 ${viewMode === 'list' ? 'flex-1' : ''}`}>
        ...
        <p className={`text-gray-600 mb-3 ${viewMode === 'list' ? 'line-clamp-2' : 'line-clamp-2'}`}>
          {asset.description}
        </p>
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-sm text-gray-600">{asset.downloads.toLocaleString()}</span>
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-4">
            ...
            <Select
              ...
              onValueChange={(value) => handleFilterChange('priceFilter', value)}
            >
